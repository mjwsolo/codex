...
pub(crate) fn print_auto_backup_start(startup_error: &LocalStateDbStartupError) {
    eprintln!("localcode couldn't start because its local database appears to be damaged.");
    eprintln!("Moving the damaged local database aside so localcode can rebuild it from saved data.");
...
pub(crate) fn confirm_fresh_start_rebuild(
    ...
    eprintln!("localcode rebuilt its local database.");
    eprintln!(
        "localcode detected a damaged local database, moved it into a backup folder, and will continue startup with a fresh database."
    );
...
pub(crate) fn print_diagnostic_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("localcode couldn't start because its local database appears to be damaged.");
    eprintln!("Run `localcode doctor` to check your setup and get next-step guidance.");
...
pub(crate) fn print_locked_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("localcode couldn't start because another localcode process is using its local data.");
    eprintln!("Quit any other copies of localcode that may still be running, then try again.");
    print_technical_details(startup_error);
}

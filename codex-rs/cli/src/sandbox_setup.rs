...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run localcode after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
    ...
    /// Use the current Windows user as the localcode user.
    #[arg(
        long = "current-user",
    ...
    /// CODEX_HOME for the localcode user. Required with --user.
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`localcode sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load target user's localcode config for sandbox provisioning")?;

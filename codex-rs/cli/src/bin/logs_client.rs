...
#[command(about = "Tail localcode logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
...
mod tests {
    ...
    /// Explicit database selection must not parse an overridden localcode home.
    #[tokio::test]
    async fn direct_db_skips_codex_home_config() {
        let codex_home = tempfile::tempdir().expect("create localcode home");

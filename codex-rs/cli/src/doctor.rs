...
/// Options for building a local localcode diagnostic report.
///
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load localcode config")
}
...
/// JSON support report emitted by `localcode doctor --json`.
///
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        ...
        if env::var_os("CODEX_MANAGED_PACKAGE_ROOT").is_none() {
            ...
            remediation = Some(
                "Reinstall or update localcode so the JS shim provides CODEX_MANAGED_PACKAGE_ROOT."
                    .to_string(),
            );
        }
    }
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run localcode login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no localcode credentials were found",
        )
        .details(details)
        .remediation("Run localcode login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run localcode login again."),
    }
}
...
mod tests {
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running localcode."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running localcode.".to_string())
        );
    }

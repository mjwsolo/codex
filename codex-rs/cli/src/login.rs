...
/// Installs a small file-backed tracing layer for direct `localcode login` flows.
///
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `localcode login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | localcode login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv CODEX_ACCESS_TOKEN | localcode login --with-access-token`.",
...
/// `localcode login` working in environments where device-code may be disabled/feature-gated.
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let codex_home = tempdir().expect("create temporary localcode home");
        login_with_api_key(
            codex_home.path(),
            "sk-existing",

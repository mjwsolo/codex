...
#[command(
    version,
    override_usage = "localcode exec [OPTIONS] [PROMPT]\n       localcode exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
...
struct ResumeArgsRaw {
    ...
    // so "localcode resume --last <prompt>" treats the positional as a prompt, not a session id.

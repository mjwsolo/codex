...
/// Minimal launcher that initializes the localcode app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "localcode", version, about = "Bootstrap localcode app-server", long_about = None)]
struct Cli {
    /// Path to the `localcode` CLI binary. When set, requests use stdio by
    /// spawning `localcode app-server` as a child process.
    ...
    /// Forwarded to the `localcode` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `localcode app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/codex-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `localcode app-server --listen`.
    ...
    /// Send a user message through the localcode app-server.
    SendMessage {
        /// User message to send to localcode.
        user_message: String,
    },
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a localcode-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// Fetch the current account rate limits from the localcode app-server.
    GetAccountRateLimits,
    /// List the available models from the localcode app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the localcode app-server.
...
fn serve(codex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    ...
    println!("started localcode app-server");
...
impl CodexClient {
    ...
    fn spawn_stdio_with_env(
        ...
        let stdin = codex_app_server
            .stdin
            .take()
            .context("localcode app-server stdin unavailable")?;
        let stdout = codex_app_server
            .stdout
            .take()
            .context("localcode app-server stdout unavailable")?;
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    ...
                    title: Some("localcode Toy App Server".to_string()),
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to localcode app-server")?;
                    return Ok(());
                }
                bail!("localcode app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from localcode app-server")?;
                if bytes == 0 {
                    bail!("localcode app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[localcode app-server exited: {status}]");
            return;
        }

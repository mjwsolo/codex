...
/// Selects whether WebSocket TLS follows localcode custom-CA policy or Tungstenite defaults.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum WebSocketTlsMode {
    /// Build an explicit TLS configuration from native roots and configured localcode custom CAs.
    ExplicitCodexTls,
...
impl WebSocketConnector {
    /// Creates a connector using native roots and any configured localcode custom CA bundle.
    pub fn new(
        http_client_factory: &HttpClientFactory,
    ...
    /// Creates a connector with explicit localcode TLS or the transport's existing TLS defaults.
    ///
    /// HTTPS proxy connections still build localcode TLS configuration when they establish their

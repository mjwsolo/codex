...
pub struct ConfigToml {
    ...
    /// sanctioned by localcode will likely degrade model performance.
    ...
    /// file: Use a file in the localcode home directory.
    ...
    /// Directory where localcode stores the SQLite state DB.
    /// Defaults to `$CODEX_SQLITE_HOME` when set. Otherwise uses `$CODEX_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where localcode writes log files. Setting this value explicitly
    ...
    /// Optional product SKU forwarded on host-owned localcode Apps MCP requests.

...
pub(super) async fn load_config_layers_internal(
    ...
    let startup_warnings = if ignore_default_managed_config {
        ...
        {
            Ok(_) => vec![format!(
                "Ignoring deprecated managed config file at {}; CODEX_HOME/managed_config.toml is no longer supported on Windows. Use %ProgramData%\\OpenAI\\localcode\\requirements.toml for enforced settings or config.toml for defaults.",

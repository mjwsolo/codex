...
- the user wants localcode to see the updated plugin without manually editing marketplace files
...
2. Update the plugin manifest to a single localcode cachebuster suffix:
...
outside localcode depends on a specific token:
...
`~/.agents/plugins/marketplace.json`. You do not need `localcode plugin marketplace add` for that
path, and `localcode plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
6. Prompt the user to use a new thread to try the updated plugin, so that localcode picks up new skills
   and tools.
...
Replace the existing localcode cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `localcode plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `localcode plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by localcode.
...
- Use `localcode plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `localcode plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `localcode plugin add <plugin-name>@<marketplace-name>`.

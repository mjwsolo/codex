---
name: skill-creator
description: Create or update a localcode skill with appropriately scoped instructions and any needed supporting resources.
metadata:
...
Create skills that give localcode useful, non-obvious guidance without constraining unrelated work.

## Core Principles

**Assume localcode is already capable.** Include only information that changes its decisions or improves its work. Remove generic advice, repeated instructions, speculative edge cases, and examples that do not materially clarify the task.

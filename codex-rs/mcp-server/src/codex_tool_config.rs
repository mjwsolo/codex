...
/// Client-supplied configuration for a `localcode` tool-call.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, Default)]
#[serde(rename_all = "kebab-case", deny_unknown_fields)]
#[schemars(deny_unknown_fields)]
pub struct CodexToolCallParam {
    /// The *initial user prompt* to start the localcode conversation.
    pub prompt: String,
...
/// Builds a `Tool` definition (JSON schema etc.) for the localcode tool-call.
pub(crate) fn create_tool_for_codex_tool_call_param() -> Tool {
    ...
    let input_schema = create_tool_input_schema(schema, "localcode tool schema should serialize");

    Tool::new(
        "codex",
        "Run a localcode session. Accepts configuration parameters matching the localcode Config struct.",
        input_schema,
    )
    .with_title("localcode")
...
impl CodexToolCallParam {
    /// Returns the initial user prompt to start the localcode conversation and the
...
pub struct CodexToolCallReplyParam {
    ...
    /// The thread id for this localcode session.
    ...
    /// The *next user prompt* to continue the localcode conversation.
    pub prompt: String,
}
...
pub(crate) fn create_tool_for_codex_tool_call_reply_param() -> Tool {
    ...
    let input_schema = create_tool_input_schema(schema, "localcode reply tool schema should serialize");

    Tool::new(
        "codex-reply",
        "Continue a localcode conversation by providing the thread id and prompt.",
        input_schema,
    )
    .with_title("localcode Reply")
    .with_raw_output_schema(codex_tool_output_schema())
}
...
mod tests {
    ...
    fn verify_codex_tool_json_schema() {
        ...
        let expected_tool_json = serde_json::json!({
          "description": "Run a localcode session. Accepts configuration parameters matching the localcode Config struct.",
          "inputSchema": {
            ...
            "properties": {
              ...
              "prompt": {
                "description": "The *initial user prompt* to start the localcode conversation.",
                "type": "string"
              },
              "sandbox": {
    ...
    fn verify_codex_tool_reply_json_schema() {
        ...
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a localcode conversation by providing the thread id and prompt.",
          "inputSchema": {
            "properties": {
              "conversationId": {
              ...
              "prompt": {
                "description": "The *next user prompt* to continue the localcode conversation.",
                "type": "string"
              },
              "threadId": {
                "description": "The thread id for this localcode session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [

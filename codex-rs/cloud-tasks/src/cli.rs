...
pub enum Command {
    /// Submit a new localcode Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a localcode Cloud task.
    Status(StatusCommand),
    /// List localcode Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a localcode Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a localcode Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in localcode Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `localcode cloud` to browse).
    ...
    /// Git branch to run in localcode Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// localcode Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// localcode Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// localcode Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,

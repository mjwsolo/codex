...
mod tests {
    ...
    fn render_replaces_placeholders_with_and_without_whitespace() {
        let rendered = render(
            ...
            [("name", "localcode"), ("place", "codex-rs")],
        )
        .unwrap();

        assert_eq!(
            rendered,
            "Hello, localcode. You are in codex-rs. localcode is repeated."
        );
    }
    ...
    fn parsed_templates_can_be_reused() {
        ...
        assert_eq!(
            template.render([("greeting", "Hello"), ("name", "localcode")]),
            Ok("Hello, localcode!".to_string())
        );
        assert_eq!(
    ...
    fn render_supports_literal_delimiter_escapes() {
        ...
        assert_eq!(
            rendered,
            "literal open: {{, literal close: }}, value: localcode"
        );
    }
    ...
    fn render_errors_when_extra_value_is_provided() {
        ...
        assert_eq!(
            template.render([("name", "localcode"), ("unused", "extra")]),
    ...
    fn render_errors_when_duplicate_value_is_provided() {
        ...
        assert_eq!(
            template.render([("name", "localcode"), ("name", "other")]),
    ...
    fn render_function_wraps_parse_errors() {
        let err = render("Hello, }} world.", [("name", "localcode")]).unwrap_err();
    ...
    fn render_function_wraps_render_errors() {
        let err = render("Hello, {{ name }}.", [("extra", "localcode")]).unwrap_err();

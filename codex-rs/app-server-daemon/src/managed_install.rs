...
pub(crate) async fn resolved_managed_codex_bin(codex_bin: &Path) -> Result<PathBuf> {
    fs::canonicalize(codex_bin).await.with_context(|| {
        format!(
            "failed to resolve managed localcode binary {}",
            codex_bin.display()
        )
    })
}
...
pub(crate) async fn managed_codex_version(codex_bin: &Path) -> Result<String> {
    ...
    let output = command.arg("--version").output().await.with_context(|| {
        format!(
            "failed to invoke managed localcode binary {}",
            codex_bin.display()
        )
    })?;
    if !output.status.success() {
        return Err(anyhow!(
            "managed localcode binary {} exited with status {}",
            codex_bin.display(),
            output.status
        ));
    }
    ...
    let stdout = String::from_utf8(output.stdout).with_context(|| {
        format!(
            "managed localcode version was not utf-8: {}",
            codex_bin.display()
        )
    })?;
...
fn parse_codex_version(output: &str) -> Result<String> {
    let version = output
        ...
        .ok_or_else(|| anyhow!("managed localcode version output was malformed"))?;

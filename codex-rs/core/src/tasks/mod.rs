...
///
/// Implementations encapsulate a specific localcode workflow (regular chat,
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; localcode will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;

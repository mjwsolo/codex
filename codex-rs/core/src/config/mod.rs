...
impl Permissions {
    ...
    /// selection. Internal localcode-only writable roots are intentionally excluded.
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, localcode will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - localcode
    ...
    ///
    /// ```toml
    /// notify = ["notify-send", "localcode"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send localcode '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the localcode home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that localcode can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by localcode unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/codex/blob/main/codex-rs/rmcp-client/src/oauth.rs#L2
    /// file: CODEX_HOME/.credentials.json
    ///       This file will be readable to localcode and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, localcode will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all localcode state (defaults to `~/.codex` but can be
    /// overridden by the `CODEX_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Resolved configuration shared by all localcode SQLite databases.
    pub sqlite: codex_state::SqliteConfig,

    /// Directory where localcode writes log files (defaults to `$CODEX_HOME/log`).
    pub log_dir: PathBuf,
    ...
    /// Path to the current localcode executable. This cannot be set in the config
    ...
    /// Whether localcode-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for localcode updates on startup and surfaces update prompts.
    /// Set to `false` only if your localcode updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across localcode product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across localcode product surfaces.
    /// Defaults to `true`.
...
impl Config {
    ...
    /// Whether Guardian may use the unmetered localcode inference endpoints.
    ...
    /// Load a default configuration for a specific localcode home without reading
    ...
    /// ignores user settings. For example, the `localcode exec` subcommand is
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
            ...
            && constrained_approval_policy.value() == AskForApproval::Never
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; localcode would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode.",
            ));
        }
        if permission_profile_was_constrained {
...
/// Returns the path to the localcode configuration directory, which can be
...
/// Returns the path to the folder where localcode logs are stored. Does not verify

...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of localcode TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (localcode TUI):
    /// - Build this binary: `cargo build -p codex-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `localcode mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in localcode TUI, ask it to call:
...
/// Scenarios for `image_scenario`, intended to exercise localcode TUI handling of MCP image outputs.
///

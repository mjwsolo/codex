...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that localcode-managed ChatGPT auth tokens have.
    ...
    ChatgptAuthTokens {
        ...
        ///
        /// When `null`, localcode attempts to derive the plan type from access-token
...
pub enum ChatgptAuthTokensRefreshReason {
    /// localcode attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that localcode was previously using.
    ///

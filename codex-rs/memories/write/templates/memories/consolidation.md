...
The folder `{{ memory_root }}/` is a git repository managed by localcode. Read
`{{ phase2_workspace_diff_file }}` in this same folder first. It contains the git-style diff from
the previous successful Phase 2 baseline to the current worktree. It is generated by localcode for
...
- Typical workflows and tools (how they like to work, how they use localcode/agents, preferred formats)
